...
public class Binding implements Comparable<Object>, Serializable {
    ...
    private void initLocationInfo(Node node) {
        ...
        if ((bodyNode instanceof Function && ((Function) bodyNode).name == node) ||
                ...
                (bodyNode instanceof Moduler && ((Moduler) bodyNode).name == node))
        {
    ...
    public Str findDocString() {
        ...
        if (kind == Kind.CLASS) {
            ...
            while (fullNode != null && !(fullNode instanceof Moduler)) {
                fullNode = fullNode.parent;
            }
        }
        ...
        } else if (fullNode instanceof Moduler) {
            return ((Moduler) fullNode).docstring;
        } else {
            return null;
        }
    }
